...
// Recupera i dati del budget da modificare
$budget = get_budget_by_id($conn, $budget_id, $user_id);

// Se il budget non esiste o non appartiene all'utente, reindirizza
if (!$budget) {
    header("location: budgets.php?message=Budget non trovato.&type=error");
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Modifica Budget - Bearget</title>
...
<body class="text-gray-200">
    <div class="flex h-screen">
        ...
        <aside class="w-64 flex-shrink-0 bg-gray-800 p-4 flex flex-col justify-between">
            <div>
                ...
                <nav class="space-y-2">
                    ...
                    <a href="budgets.php" class="flex items-center px-4 py-2.5 text-white bg-gray-900 rounded-lg font-semibold">
        ...
        <main class="flex-1 p-6 lg:p-10 overflow-y-auto">
            <header class="mb-8">
                <h1 class="text-3xl font-bold text-white">Modifica Budget</h1>
                <p class="text-gray-400">Aggiorna il limite di spesa per la categoria <span class="font-semibold text-indigo-400"><?php echo htmlspecialchars($budget['category_name']); ?></span>.</p>
            </header>
            ...
            <div class="bg-gray-800 rounded-2xl p-6 max-w-lg mx-auto">
                <form action="update_budget.php" method="POST">
                    ...
                    <div class="space-y-6">
                        <div>
                            <label for="amount" class="block text-sm font-medium text-gray-300 mb-1">Nuovo Limite di Spesa (€)</label>
                            <input type="number" step="0.01" name="amount" id="amount" required value="<?php echo $budget['amount']; ?>" class="w-full bg-gray-700 border-gray-600 text-white rounded-lg px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                        </div>
                    </div>
                    <div class="mt-8 flex justify-end space-x-4">
                        <a href="budgets.php" class="bg-gray-600 hover:bg-gray-700 text-white font-semibold py-2 px-5 rounded-lg transition-colors">Annulla</a>
