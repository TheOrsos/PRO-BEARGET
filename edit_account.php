...
// Recupera i dati del conto da modificare
$account = get_account_by_id($conn, $account_id, $user_id);

// Se il conto non esiste o non appartiene all'utente, reindirizza
if (!$account) {
    header("location: accounts.php?message=Conto non trovato.&type=error");
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Modifica Conto - Bearget</title>
...
<body class="text-gray-200">
    <div class="flex h-screen">
        ...
        <aside class="w-64 flex-shrink-0 bg-gray-800 p-4 flex flex-col justify-between">
            <div>
                ...
                <nav class="space-y-2">
                    ...
                    <a href="accounts.php" class="flex items-center px-4 py-2.5 text-white bg-gray-900 rounded-lg font-semibold">
        ...
        <main class="flex-1 p-6 lg:p-10 overflow-y-auto">
            <header class="mb-8">
                <h1 class="text-3xl font-bold text-white">Modifica Conto</h1>
                <p class="text-gray-400">Aggiorna i dettagli del tuo conto.</p>
            </header>
                ...
                <form action="update_account.php" method="POST">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" required value="<?php echo htmlspecialchars($account['name']); ?>" class="w-full bg-gray-700 border-gray-600 text-white rounded-lg px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                        </div>
                        <div>
                            <label for="initial_balance" class="block text-sm font-medium text-gray-300 mb-1">Saldo Iniziale</label>
                            <input type="number" step="0.01" name="initial_balance" id="initial_balance" required value="<?php echo $account['initial_balance']; ?>" class="w-full bg-gray-700 border-gray-600 text-white rounded-lg px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                            <p class="text-xs text-gray-500 mt-1">Modificare questo valore influenzerà il saldo calcolato del conto.</p>
                        </div>
                    </div>
                    <div class="mt-8 flex justify-end space-x-4">
                        <a href="accounts.php" class="bg-gray-600 hover:bg-gray-700 text-white font-semibold py-2 px-5 rounded-lg transition-colors">Annulla</a>
